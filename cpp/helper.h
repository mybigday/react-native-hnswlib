...
#define JSI_ASSERT(RUNTIME, CONDITION, MESSAGE) \
  if (!(CONDITION)) throw facebook::jsi::JSError(RUNTIME, MESSAGE)

#define JSI_GET_PROPERTY_OR_DEFAULT(OBJECT, RUNTIME, PROPERTY, DEFAULT) \
  OBJECT.getProperty(RUNTIME, PROPERTY).isUndefined() ? DEFAULT : OBJECT.getProperty(RUNTIME, PROPERTY)
...
#define JSI_CREATE_PROMISE(RUNTIME, EXECUTOR, ...) \
  RUNTIME.global().getPropertyAsFunction(RUNTIME, "Promise").callAsConstructor(RUNTIME, facebook::jsi::Function::createFromHostFunction( \
    RUNTIME, \
    facebook::jsi::PropNameID::forAscii(RUNTIME, "executor"), \
    2, \
    [__VA_ARGS__](facebook::jsi::Runtime &runtime, const facebook::jsi::Value &thisValue, const facebook::jsi::Value *args, size_t argc) -> facebook::jsi::Value { \
