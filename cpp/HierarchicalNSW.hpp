...
public:
  ...
  JSI_HOST_OBJECT_CONSTRUCTOR(HierarchicalNSW) {
    ...
    if (spaceName == "l2") {
      space_ = std::make_unique<hnswlib::L2Space>(
          static_cast<size_t>(numDimensions));
  ...
  // number, randomSeed?: number, allowReplaceDeleted?: boolean): void;
  JSI_HOST_FUNCTION(initIndex) {
    JSI_ASSERT(runtime, argc >= 1, "Expected at least 1 argument");
    JSI_ASSERT(runtime, args[0].isNumber() || args[0].isObject(),
               "Expected number or object");
    if (argc >= 2)
      JSI_ASSERT(runtime, args[1].isNumber(), "Expected number");
    if (argc >= 3)
      JSI_ASSERT(runtime, args[2].isNumber(), "Expected number");
    if (argc >= 4)
      JSI_ASSERT(runtime, args[3].isNumber(), "Expected number");
    if (argc >= 5)
      JSI_ASSERT(runtime, args[4].isBool(), "Expected boolean");

    const bool use_opts = args[0].isObject();

    if (use_opts)
      JSI_ASSERT(runtime,
                 args[0].getObject(runtime).hasProperty(runtime, "maxElements"),
                 "`maxElements` is required");

    const uint32_t max_elements = use_opts
                                      ? args[0]
                                            .getObject(runtime)
                                            .getProperty(runtime, "maxElements")
                                            .getNumber()
                                      : args[0].getNumber();
    const uint32_t m =
        use_opts
            ? JSI_GET_PROPERTY_OR_DEFAULT(args[0].getObject(runtime), runtime,
                                          "m", 16)
                  .getNumber()
    ...
    try {
      index_ = std::make_unique<hnswlib::HierarchicalNSW<float>>(
          space_.get(), max_elements, m, ef_construction, random_seed,
  ...
  // readIndex(filename: string, allowReplaceDeleted?: boolean):
  // Promise<boolean>;
  JSI_HOST_FUNCTION(readIndex) {
    JSI_ASSERT(runtime, argc >= 1, "Expected at least 1 argument");
    JSI_ASSERT(runtime, args[0].isString(), "Expected string");
    if (argc >= 2)
      JSI_ASSERT(runtime, args[1].isBool(), "Expected boolean");

    std::string filename = args[0].getString(runtime).utf8(runtime);
    bool allow_replace_deleted =
        JSI_GET_ARGUMENT_OR_DEFAULT(args, runtime, 1, false).getBool();

    return JSI_CREATE_PROMISE(
        runtime,
        {
          if (index_ != nullptr)
            index_.reset();
          ...
          try {
            index_ = std::make_unique<hnswlib::HierarchicalNSW<float>>(
                space_.get(), filename, false, 0, allow_replace_deleted);
  ...
  JSI_HOST_FUNCTION(addPoint) {
    ...
    JSI_ASSERT(runtime, args[1].isNumber(), "Expected number");
    if (argc >= 3)
      JSI_ASSERT(runtime, args[2].isBool(), "Expected boolean");
    JSI_ASSERT(runtime, index_ == nullptr, "Index is not initialized");
    ...
    const bool replace_deleted =
        JSI_GET_ARGUMENT_OR_DEFAULT(args, runtime, 2, false).getBool();
  ...
  // searchKnn(queryPoint: number[], numNeighbors: number, filter?:
  // FilterFunction): {distances: number[], neighbors: number[]};
  JSI_HOST_FUNCTION(searchKnn) {
    JSI_ASSERT(runtime, argc >= 2, "Expected at least 2 arguments");
    JSI_ASSERT(runtime, args[0].isArray(), "Expected array");
    JSI_ASSERT(runtime, args[1].isNumber(), "Expected number");
    if (argc >= 3)
      JSI_ASSERT(runtime, args[2].isObject(), "Expected object");

    std::vector<float> query_vector;
    auto query_point = args[0].asArray(runtime);
    for (size_t i = 0; i < query_point.length(runtime); i++) {
      query_vector.push_back(
          query_point.getValueAtIndex(runtime, i).getNumber());
    }
    ...
    std::priority_queue<std::pair<float, hnswlib::labeltype>> result =
        index_->searchKnn(query_vector.data(), num_neighbors, filter);
    ...
    resultObject.setProperty(runtime, "distances",
                             jsi::Array(runtime, distances));
    resultObject.setProperty(runtime, "neighbors",
                             jsi::Array(runtime, neighbors));
  ...
  JSI_HOST_FUNCTION(getPoint) {
    ...
    std::vector<float> point =
        index_->getDataByLabel<float>(static_cast<size_t>(label));
